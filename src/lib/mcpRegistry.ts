...
// `<datadir>/mcp-servers.json`). In browser-dev (vite, no Tauri) there is no
// Rust side, so each call falls back to a localStorage-backed registry so the
// settings UI is still drivable headless.
