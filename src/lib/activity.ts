// Unified, DERIVED transfer feed for the Activity page.
//
// Activity is no longer a local-only capture log that knows only about the
// SENDS this app happened to broadcast. It's now derived from authoritative
// sources and merged by tx_id — so INCOMING deposits appear too:
//
...
// detail. No synthetic placeholder rows.
//
// Ported from exfer-walletd-mobile/src/lib/activity.ts and adapted to the
// desktop's rpc + history (the desktop local log is sends-only — every entry is
// a send, so there are no "received" / "recv-" local rows to reconcile).
...
export interface ActivityItem {
  ...
  /** Set when confirmed (from the indexer / get_transaction). */
  block_height?: number;
  /** Best-effort ISO timestamp. Only known for sends we logged locally
   *  (broadcast time). Confirmed-only rows from the indexer have no
   *  timestamp — the UI shows the block height instead. */
  ts?: string;
  /** Received: which of our addresses got credited. */
  toAddresses?: string[];
  /** The other party, from the indexer: senders for a received item,
   *  recipients for a sent item. Empty for pending/mempool rows until indexed. */
...
/** True when walletd reports that no indexer is wired up (-32007
 *  IndexerNotConfigured), so the caller can degrade gracefully to the mempool +
 *  local-log view instead of surfacing an error. */
 ...
 *  as unknown ("Transaction not found") — what tells a dropped/evicted transfer
 *  apart from a transient query failure (leave pending). */
...
export async function buildActivityFeed(
  ...
  //    just-broadcast send the network hasn't shown us yet. The desktop log is
  //    sends-only (every entry is a send), so there are no received/placeholder
  //    rows to reconcile.
  ...
  // Resolve authoritative status for anything the indexer hasn't confirmed.
  // The indexer is the source of truth for HISTORY (which txs touched an
  // address) but it trails the chain tip, so a transfer that already confirmed
  // and left the mempool would otherwise linger as "in mempool". The node's
  // get_transaction gives up-to-the-second status; confirmed heights are final,
  // so we cache them to avoid re-querying on every refresh.
  ...
  // A locally-recorded send the node reports as unknown ("Transaction not
  // found") is neither on chain nor in any mempool — the network dropped it.
  // Left alone it reads "in mempool" forever. Prune such zombies, but only
  // after a grace period so a fresh broadcast isn't deleted before it propagates.
  ...
  await Promise.all(
    unresolved.map(async (it) => {
      try {
        ...
        // Any other failure (transient / network) leaves the row pending.
      }
    }),
  );

  // Newest first: pending above confirmed; then by block height when both have
  // one, else by timestamp.
  const items = all
