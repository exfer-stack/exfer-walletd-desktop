...
// We never want to show the user a raw code dump, so we match known cases by
// substring (robust to the wrapping) and fall back to a generic line — keeping
// the raw text only in the console for debugging. Messages are localized via
// tStatic (reads the persisted language). Ported from the mobile wallet.
...
const PATTERNS: { test: RegExp; key: MsgKey }[] = [
  ...
  { test: /swap not configured|swap engine|set --swap-pool/i, key: "err.swapUnavailable" },
  { test: /quote expired|request a fresh quote/i, key: "err.quoteExpired" },
  {
  ...
  { test: /method not found|-32601|not supported|unsupported|unknown method/i, key: "err.oldNode" },
  {
  ...
  { test: /already (known|in mempool|exists)|duplicate|nonce/i, key: "err.duplicate" },
  { test: /fee.*(too low|below)|min(imum)? fee/i, key: "err.feeLow" },
  { test: /no file selected|cancelled|canceled/i, key: "err.cancelled" },
];
 ...
 * contextual title at the call site, e.g. `toast.error(t("snd.failedTitle"),
