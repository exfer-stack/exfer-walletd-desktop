// In-progress liquidity operations, tracked locally so an "in progress" surface
// (the swap screen, a global nav badge) can show LP adds/removes alongside
// swaps — and so they survive the user closing the liquidity sheet mid-flow
// (the deposit finishes in the background). Adds are removed when their
// on-chain status goes terminal; removes (no per-id status endpoint, settle in
// seconds) fall off by a TTL.
//
// Ported from the mobile wallet (src/lib/inflightLp.ts). The only desktop
// adaptation is the localStorage key, namespaced to this app.
...
const KEY = "exfer-walletd-desktop-inflight-lp-v1";
...
const MAX_AGE: Record<LpOp["kind"], number> = {
  add: 10 * 60_000,
  remove: 60_000,
};
...
function writeRaw(ops: LpOp[]): void {
  ...
  try {
    window.dispatchEvent(new Event("inflight-lp"));
  } catch {
    /* no window (shouldn't happen in the webview) */
  }
}
...
export function getLpOps(): LpOp[] {
  ...
  return readRaw().filter(
    (o) => o && o.id && now - o.startedAt < (MAX_AGE[o.kind] ?? 60_000),
  );
}
