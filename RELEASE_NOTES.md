...
- Swaps now finish reliably. A BNB→EXFER swap could get stuck on "Sending your
  ...
  when the pool had in fact taken the other side. The app now re-checks the pool
  and the chain directly while a swap is in progress, so it completes a settled
  swap and picks up a matched one on its own — including swaps that were already
  stuck (they recover the next time you open the app). Your funds were always
  safe on-chain; this fixes the screen catching up to reality.

- Honest in-progress wording. A swap that's still confirming no longer claims it
  "didn't match" or tells you to close the page — closing during the confirmation
