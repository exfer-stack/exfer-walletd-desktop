...
//! device itself. The desktop normally could download + exec the standalone
//! `exfer-agent-miner` binary, but running IN-PROCESS keeps the miner a
//! controllable handle (start/stop/status) living in Tauri state and matches
//! the mobile wallet byte-for-byte, so the agent's `exfer_mine_*` tools behave
//! identically on both. It drives the same Argon2id PoW + stratum loop as
//! `exfer-agent-miner`, using only pure-Rust crates (exfer-pow = Argon2id;
//! exfer-stratum = rustls TLS). No subprocess, no downloaded binary, GPU/CUDA
//! off.
//!
