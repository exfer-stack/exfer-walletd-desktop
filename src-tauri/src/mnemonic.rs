...
//!
//! - **Standard (BIP39)** — what exfer.dev (the web wallet), the Flutter
//!   wallet, and the Exfer mobile wallet produce. Pinned by a test vector
//!   below. `seed = BIP39 mnemonic-to-seed, empty passphrase` (PBKDF2-HMAC-
//!   SHA512); `secret = SHA-256("EXFER-MNEMONIC-ED25519-V1" || seed)`.
//!
//! - **Legacy** — what walletd's own `import_mnemonic` does (a per-address
//!   "private-key backup phrase"): the 24 words ARE the raw key, i.e.
...
//! This is the exact derivation the mobile wallet uses, so a phrase imported on
//! one lands on the same address on the other.

use exfer_walletd::store::Signer;
use sha2::{Digest, Sha256};

/// Domain tag exfer.dev / the Flutter + mobile wallets prepend to the BIP39
/// seed before hashing to the ed25519 secret. Must match their bundle exactly.
const STD_DOMAIN: &[u8] = b"EXFER-MNEMONIC-ED25519-V1";

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum Scheme {
    /// exfer.dev / Flutter / mobile wallet — a real BIP39 HD seed phrase.
    Standard,
...
mod tests {
    ...
    // address (also confirmed against the Flutter + mobile wallets). If the
    // standard derivation ever drifts from exfer.dev, this fails loudly. It is
    // the SAME vector the mobile wallet pins, so desktop and mobile stay in
    // lockstep.
