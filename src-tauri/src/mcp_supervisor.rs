...
//! `uvx exfer-mcp` Python sidecar, but the tool layer is now native (in lockstep
//! with the mobile app) so the agent has no uvx/Python dependency.
...
/// never spawns the `uvx exfer-mcp` sidecar. Kept for reference; remove once the
/// native path has soaked.
#[allow(dead_code)]
