...
//! agent's wallet tools have zero environment dependency. The `exfer` server is
//! implicit, always enabled, and not user-removable, and the `exfer_` name
//! prefix is RESERVED for it (see [`is_reserved_tool_name`]).
//!
...
/// Read a tool's bearer secret from the OS keychain (host-side injection — the
...
fn tool_auth(auth_ref: &Option<String>) -> Option<String> {
    let r = auth_ref.as_ref()?;
    if r.trim().is_empty() {
        return None;
    }
    crate::secrets::get_passphrase(r).ok().flatten()
}

/// POST one op to a plain-HTTP tool endpoint, injecting the bearer secret
/// host-side. Returns the parsed JSON body.
async fn httptool_post(url: &str, auth: &Option<String>, body: Value) -> Result<Value, String> {
    let client = tool_http_client()?;
    let mut req = client
        .post(url)
        .header("content-type", "application/json")
        .body(body.to_string());
    if let Some(tok) = tool_auth(auth) {
...
async fn httptool_list(url: &str, auth: &Option<String>) -> Result<Vec<Value>, String> {
    let v = httptool_post(url, auth, json!({ "op": "list" })).await?;
    let tools = v
        .get("tools")
    ...
    for cfg in registry.iter().filter(|c| c.enabled) {
        match cfg.transport.as_str() {
            "httptool" => {
                ...
                match httptool_list(url, &cfg.auth_ref).await {
                    Ok(tools) => {
                        for t in tools {
...
async fn route_map(app: &AppCtx, mcp: &McpCtx) -> HashMap<String, String> {
        ...
        .filter(|c| c.enabled && c.transport == "httptool")
    {
        if let Some(url) = cfg.url.as_deref().filter(|u| !u.trim().is_empty()) {
            if let Ok(tools) = httptool_list(url, &cfg.auth_ref).await {
                for t in tools {
...
pub async fn mcp_call_tool(
    ...
    match cfg.transport.as_str() {
        "httptool" => {
            ...
            httptool_call(url, &cfg.auth_ref, &name, args).await
        }
        "http" => {
